...
import modules.mouse_track_helper_func_class as misc
...
def plot_arena_grid(ax,G_or_L,X_arena_lim,Y_arena_lim,line_color=None,line_style='--',show_grid_lines=True,grid_alpha=0.8,cmap_grid=None):
    """
    Plot a square grid centered on the arena's center, with L squares per row and per column.

    Parameters:
    ax              (matplotlib.axes.Axes, optional): The Matplotlib axis to plot on. If None, the current axis (plt.gca()) is used.
    G_or_L          (int or numpy.ndarray)          : Either the size of the square grid side (L) as an integer or the grid matrix (G) generated by the mouse walk.
    X_arena_lim     (tuple)                         : The limits of the arena in the X direction.
    Y_arena_lim     (tuple)                         : The limits of the arena in the Y direction.
    line_color      (str or array-like, optional)   : Color of the grid lines. If None, the default color is used. Default is None.
    line_style      (str, optional)                 : Style of the grid lines (e.g., '--' for dashed lines). Default is '--'.
    show_grid_lines (bool, optional)                : Whether to show grid lines. Default is True.
    grid_alpha      (float, optional)               : Transparency level of the grid (0.0 to 1.0). Default is 0.8.
    cmap_grid       (str or None, optional)         : Colormap for the grid. If None, the default colormap is used. Default is None.

    Returns:
    list: A list of artists representing the plotted grid, including lines and the grid matrix if provided.
    ...
    >>> X_arena_lim = (-5, 5)
    >>> Y_arena_lim = (-5, 5)
    >>> l = plot_arena_grid(ax, G_or_L, X_arena_lim, Y_arena_lim, line_color='blue', line_style='-', show_grid_lines=True, grid_alpha=0.8, cmap_grid='viridis')
    >>> plt.show()
    """
...
def get_gradient(N=None,color='red',cmap_name=None,verbose=True):
    if misc.exists(cmap_name):
        ...
        if type(color) is str:
            ...
            if callable(color):
                ...
                raise ValueError('color is not a known gradient, so it must be a callable colormap-like function')
    if misc.exists(N):
        return f(numpy.linspace(0.0,1.0,N))
    else:
        return f
...
def plot_arena_sketch(track,showAllEntrances=False,arenaPicture=False,ax=None,showHoles=True,showArenaCircle=True,holesColor=None,showStart=False,startArgs=None,bgCircleArgs=None,holesArgs=None):
    ...
    if has_arena_pic_to_plot(arenaPicture):
        ...
        if type(arenaPicture) is bool:
            warnings.warn('arenaPicture bool value -- parameter not implemented')
    ...
    if showAllEntrances:
        warnings.warn('showAllEntrances not implemented')
        #kk = 0
...
def _get_label(args,label_arg_name='label',label_args_arg_name=None):
    """
    Retrieve label and label arguments from a given dictionary of arguments.

    Parameters:
    args                (dict)         : Dictionary of arguments from which to retrieve the label and label arguments.
    label_arg_name      (str, optional): The key name for the label argument in the dictionary. Default is 'label'.
    label_args_arg_name (str, optional): The key name for the label arguments in the dictionary. If None, defaults to label_arg_name + 'Args'.

    Returns:
    tuple: A tuple containing:
        - label (str or None): The label retrieved from the dictionary, or None if not found.
        - label_args (dict): A dictionary of label arguments retrieved from the dictionary, or an empty dictionary if not found.

    Example:
    >>> args = {'label': 'Start', 'labelArgs': {'fontsize': 12, 'color': 'blue'}}
    >>> label, label_args = _get_label(args)
    >>> print(label)  # Output: 'Start'
    >>> print(label_args)  # Output: {'fontsize': 12, 'color': 'blue'}
    """
    label_args_arg_name = label_args_arg_name if misc.exists(label_args_arg_name) else label_arg_name+'Args'
    if misc.exists(args):
        return args.pop(label_arg_name,None), _get_kwargs(args.pop(label_args_arg_name,None))
    return None,dict()
...
def plot_all_tracks_2targets(track_complete,hole_horizon,time_delay_after_food,traj1Args=None,traj2Args=None,
    ...
    track_complete_rot = copy.deepcopy(track_complete)

    if trim_trajectories:
        warnings.warn('trim_trajectories not implemented')
